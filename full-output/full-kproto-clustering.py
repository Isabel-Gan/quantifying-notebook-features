...
# load the data
md_df = pd.read_pickle('full_markdown_group.pkl')
no_md_df = pd.read_pickle('full_no_markdown_group.pkl')

# clear the first column in both groups
md_df = md_df.drop(['nb_id,', 'repo_id'], axis = 1)
no_md_df = no_md_df.drop(['nb_id', 'repo_id'], axis = 1)
...
for k in K:
    ...
    clusters = kproto.fit_predict(md_df, categorical = [0, 1, 2, 3, 9, 10, 11, 13, 15, 19, 20, 22, 24, 25])
...
for k in K:
    print("clustering with " + str(k) + " clusters")
    kproto = KPrototypes(n_clusters = k, init = 'Cao', n_jobs = 4, verbose = 0)
    clusters = kproto.fit_predict(no_md_df, categorical = [0, 5, 6, 11, 12, 14, 15, 16])
